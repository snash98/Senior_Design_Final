...
def main():
        ...
        db.commit()
        db.close()
        snap =str(snap).strip('()').strip("''").strip(",").strip("''")
        print(snap)
        os.system("cd tf/ec2 && terraform init && terraform apply -var 'aws_access_key='%s -var 'aws_secret_key='%s -var 'snapshot_id='%s -var 'vpc_id='%s -var 'subnet_id='%s -auto-approve" % (access_key, secret_key, snap, vpc_id, sub))
